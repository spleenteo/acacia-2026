...
The visual language ("Japan Fish") lives in a single source of truth: the **"Styling — Japan Fish"** section of `CLAUDE.md`, backed by the `@theme` tokens in `src/app/global.css`. Consult those — do not duplicate palette/typography values here (they drift).
...
| GalleryImage\* | —       | Yes       | `ImageGallery/fragment.ts` shared fragment (district masonry + WhatWeLove); grid component removed |
